...
2. **TestGitStatusOperations** (6 tests) - Status tracking and file detection
3. **TestGitStagingOperations** (6 tests) - File staging management
4. **TestGitCommitOperations** (4 tests) - Commit creation and validation
5. **TestCheckpointCommitCreation** (5 tests) - Automatic checkpoint commits
6. **TestWorktreeOperations** (4 tests) - Worktree detection
7. **TestGitOperationsEdgeCases** (4 tests) - Edge case handling
...
2. **TestSessionLifecycleManagerInitialization** (2 tests) - Manager initialization
3. **TestSessionLifecycleDirectorySetup** (4 tests) - Directory structure setup
4. **TestSessionProjectContextAnalysis** (4 tests) - Project indicator detection
5. **TestSessionQualityScoring** (1 test) - Quality score delegation
6. **TestSessionCheckpointOperations** (1 test) - Checkpoint workflow
7. **TestSessionEndOperations** (1 test) - Session end workflow
8. **TestSessionPreviousSessionInfo** (2 tests) - Session file parsing
9. **TestSessionStatusQuery** (1 test) - Status queries
...
2. PWD assertion mismatch → Flexible path assertion
3. Project context KeyError → Correct key names (has_pyproject_toml)
4. Mock patch AttributeError → Correct module path
5. Return type mismatch → Expect dict | None, not SessionInfo
6. Session file format → Bold markdown keys
7. Missing total_score key → Added to mock responses
8. Missing breakdown key → Added complete structure
9. Return structure mismatch → Nested summary dict handling
