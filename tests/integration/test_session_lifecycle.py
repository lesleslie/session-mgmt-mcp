...
class TestSessionLifecycleIntegration(AsyncTestCase):
    ...
    async def test_complete_session_workflow(
        self, mock_session_permissions, isolated_database, temporary_project_structure
    ):
        ...
        search_result = await quick_search(
            query="session initialization testing", min_score=0.5
        )
        assert isinstance(
    ...
    async def test_session_error_recovery(
        self, mock_session_permissions, isolated_database
    ):
    ...
    async def test_permission_system_integration(
        self, mock_session_permissions, temporary_project_structure
    ):
    ...
    async def test_concurrent_session_operations(
        ...
        tasks = [
            asyncio.create_task(
                store_reflection(
                    content=f"Concurrent reflection {i}", tags=[f"test-{i}"]
                )
            )
            for i in range(5)
        ]
    ...
    async def test_database_isolation_between_sessions(self, isolated_database):
        ...
        session1_reflection = await store_reflection(
            content="Session 1 exclusive data", tags=["session-1"]
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                ...
                search_result = await quick_search(
                    query="Session 1 exclusive", project="test"
                )
                assert isinstance(search_result, str | dict)
    ...
    async def test_session_performance_metrics(
        self, performance_metrics_collector, temporary_project_structure
    ):
...
class TestMCPToolRegistration:
    ...
    async def test_tool_error_handling(self, isolated_database):
        ...
        result = await quick_search(
            query="test", project="/invalid/path/that/does/not/exist"
        )
